...
function FinPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data: r } = await supabase
        .from("reservations")
        .select("id,depart,destination,statut_course,prix_final,distance_reelle_km,duree_reelle_min,chauffeur_id,prenom,nom,email,telephone,paiement")
        .eq("id", id)
        .single();
      ...
      if (av) { setAvisExiste(true); setNote(av.note); setAvisEnvoye(true); }
  ...
  const soumettreAvis = useCallback(async () => {
    ...
    try {
      ...
      // Recalculer note moyenne chauffeur
      if (chauffeur) {
        const { data: allAvis } = await supabase
          .from("avis")
          .select("note")
          .eq("reservation_id", id);
        // Simple update optimiste
  ...
  const envoyerRecu = useCallback(async () => {
    if (!resa || recuLoading) return;
    setRecuLoading(true);
    try {
      // Appel à une Edge Function Supabase (à créer côté back)
      await (supabase as any).functions.invoke("send-recu", {
        body: { reservation_id: id },
      });
      setRecuMsg("✅ Reçu envoyé à " + resa.email);
    } catch {
      setRecuMsg("❌ Erreur lors de l'envoi");
    }
  ...
  useEffect(() => {
    ...
    const drawMap = async () => {
      ...
      const [fromCoord, toCoord] = await Promise.all([
        geocode(resa.depart),
        geocode(resa.destination),
      ]);

      if (!fromCoord || !toCoord) return;

      // Marqueurs
      const mkIcon = (emoji: string, color: string) => L.divIcon({
        className: "",
      ...
      L.marker(fromCoord, { icon: mkIcon("🟢", "rgba(34,197,94,0.85)") }).addTo(map)
        .bindPopup(`<b>Départ</b><br>${resa.depart}`);
      L.marker(toCoord, { icon: mkIcon("📍", "rgba(245,200,66,0.85)") }).addTo(map)
        .bindPopup(`<b>Arrivée</b><br>${resa.destination}`);

      // Tracé itinéraire
      const coords = await getPolyline(fromCoord, toCoord);
      if (coords.length > 0) {
        // Ombre derrière la ligne
        L.polyline(coords, { color: "#000", weight: 7, opacity: 0.3 }).addTo(map);
        L.polyline(coords, { color: "#f5c842", weight: 4, opacity: 0.9 }).addTo(map);
      }
  ...
  const memeTrajet = () => {
    if (!resa) return;
    // Passer les adresses via query params
    navigate({ to: `/reserver?depart=${encodeURIComponent(resa.depart)}&destination=${encodeURIComponent(resa.destination)}` });
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0a0a14", fontFamily: "'DM Sans',sans-serif", overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
      <style>{`
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "24px 20px calc(40px + env(safe-area-inset-bottom,0px))", display: "flex", flexDirection: "column", gap: 20, animation: "fadeIn 0.4s ease" }}>

        {/* ── Header ── */}
        <div style={{ textAlign: "center", paddingTop: 12 }}>
          <div style={{ fontSize: 64, marginBottom: 12, animation: "checkPop 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>🏁</div>
          <h1 style={{ fontFamily: "'Clash Display',sans-serif", fontWeight: 700, fontSize: 28, color: "#f5f5f5", margin: "0 0 6px" }}>Course terminée !</h1>
          <p style={{ color: "#64748b", fontSize: 15, margin: 0 }}>Merci d'avoir voyagé avec Taxi City Bordeaux</p>
        </div>

        {/* ── Récap course ── */}
        <div style={{ background: "linear-gradient(135deg,#1a1408,#211808)", border: "1px solid rgba(245,200,66,0.25)", borderRadius: 20, padding: 20, boxShadow: "0 4px 24px rgba(245,200,66,0.08)" }}>
          {/* Prix */}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {[
              { icon: "📏", label: "Distance", value: resa.distance_reelle_km != null ? `${resa.distance_reelle_km} km` : "—" },
              { icon: "⏱", label: "Durée", value: resa.duree_reelle_min != null ? formatDuree(resa.duree_reelle_min) : "—" },
              { icon: "🛣️", label: "Trajet", value: "Terminé" },
            ].map(({ icon, label, value }) => (
              <div key={label} style={{ background: "rgba(0,0,0,0.25)", borderRadius: 14, padding: "12px 10px", textAlign: "center" }}>
                <div style={{ fontSize: 20, marginBottom: 4 }}>{icon}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#f5f5f5" }}>{value}</div>
                <div style={{ fontSize: 10, color: "#64748b", marginTop: 2, textTransform: "uppercase", letterSpacing: "0.04em" }}>{label}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 16, display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: "#f5f5f5", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 8 }}>{resa.depart}</div>
              <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 2 }}>Arrivée</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#f5f5f5", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{resa.destination}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#111120", borderRadius: 20, border: "1px solid #2a2a4a", overflow: "hidden" }}>
          <button
            ...
            style={{ width: "100%", padding: "16px 18px", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", color: "#f5f5f5" }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 20 }}>🗺️</span>
              <span style={{ fontWeight: 700, fontSize: 15 }}>Revoir l'itinéraire</span>
            </div>
            <span style={{ fontSize: 18, color: "#64748b", transition: "transform 0.2s", transform: showMap ? "rotate(180deg)" : "rotate(0)" }}>▾</span>
          </button>
          {showMap && (
            <div ref={mapRef} style={{ height: 240, borderTop: "1px solid #2a2a4a" }} />
          )}
        </div>
        ...
        {chauffeur && (
          <div style={{ background: "#111120", borderRadius: 20, border: "1px solid #2a2a4a", padding: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(245,200,66,0.12)", border: "2px solid rgba(245,200,66,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0, overflow: "hidden" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "#f5c842", marginTop: 2 }}>
                  {"★".repeat(Math.round(chauffeur.note_moyenne))} <span style={{ color: "#64748b" }}>{chauffeur.note_moyenne.toFixed(1)}</span>
                </div>
              </div>
            </div>
            ...
            {avisEnvoye ? (
              <div style={{ textAlign: "center", padding: "16px 0" }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 16, color: "#22c55e", marginBottom: 4 }}>Merci pour votre avis !</div>
              ...
              <>
                ...
                <div style={{ textAlign: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 13, color: displayNote ? "#f5c842" : "#64748b", fontWeight: 600, minHeight: 20, display: "inline-block" }}>
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: 4, marginBottom: 16 }}>
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button
                      ...
                      onClick={() => setNote(n)}
                      style={{ fontSize: 36, opacity: n <= displayNote ? 1 : 0.25, filter: n <= displayNote ? "none" : "grayscale(1)", transform: n === displayNote ? "scale(1.2)" : "scale(1)" }}
                    >
                      ★
                    </button>
                  ))}
                ...
                <textarea
                  ...
                  maxLength={300}
                  rows={3}
                  style={{ width: "100%", background: "#1a1a2e", border: "2px solid #2a2a4a", borderRadius: 14, padding: "12px 14px", color: "#f5f5f5", fontSize: 14, marginBottom: 12 }}
                />
        ...
        <div style={{ background: "#111120", borderRadius: 20, border: "1px solid #2a2a4a", padding: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>Envoi vers {resa.email}</div>
            </div>
          </div>
          {recuMsg && (
            <div style={{ padding: "10px 14px", background: recuMsg.startsWith("✅") ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)", border: `1px solid ${recuMsg.startsWith("✅") ? "rgba(34,197,94,0.3)" : "rgba(239,68,68,0.3)"}`, borderRadius: 12, color: recuMsg.startsWith("✅") ? "#22c55e" : "#ef4444", fontSize: 13, marginBottom: 12 }}>
              {recuMsg}
            </div>
          )}
          <button
            ...
            style={{ width: "100%", height: 48, background: "#1a1a2e", border: "2px solid #2a2a4a", borderRadius: 14, color: "#f5f5f5", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
          >
            {recuLoading ? "Envoi en cours…" : "📨 Recevoir le reçu par email"}
          </button>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <button
            onClick={memeTrajet}
            style={{ padding: "16px 12px", background: "linear-gradient(135deg,#f5c842,#e6a800)", border: "none", borderRadius: 16, color: "#0a0a14", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minHeight: 72 }}
          >
          ...
          <button
            ...
            style={{ padding: "16px 12px", background: "#1a1a2e", border: "2px solid #2a2a4a", borderRadius: 16, color: "#f5f5f5", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minHeight: 72 }}
          >
            <span style={{ fontSize: 22 }}>➕</span>
            Nouvelle course
          </button>
        ...
        <button
          ...
          style={{ width: "100%", padding: "14px", background: "none", border: "1px solid #2a2a4a", borderRadius: 14, color: "#64748b", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
        >
