...
function ReservationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate({ to: "/suivi/$id", params: { id: inserted.id } });
    } catch (err: any) {
      setSending(false);
        ...
        >
          ...
          >
            ...
            <a
              href={(() => {
                const lines = [
                  `🚖 *Nouvelle réservation Taxi City Bordeaux*`,
                  ``,
                  `👤 ${f.prenom} ${f.nom}`,
                  `📱 ${f.phone}`,
                  `✉️ ${f.email}`,
                  ...
                  `👥 Passagers : ${f.passagers}`,
                  `🧳 Bagages : ${f.bagages}`,
                  `💳 Paiement : ${f.paiement === "cb" ? "Carte bancaire" : "Espèces"}`,
                  orsResult
                    ? `💶 Tarif estimé : ${prixAller.toFixed(2)} € (${orsResult.distanceKm.toFixed(1)} km)`
                    : "",
                ]
                  .filter(Boolean)
                  .join("\n");
                return `https://wa.me/33XXXXXXXXX?text=${encodeURIComponent(lines)}`;
              })()}
              target="_blank"
