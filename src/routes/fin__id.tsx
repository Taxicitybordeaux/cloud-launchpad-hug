  ...
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      const poll = setInterval(() => {
        if ((window as any).jspdf?.jsPDF) { clearInterval(poll); resolve(); }
      }, 50);
      setTimeout(() => { clearInterval(poll); reject(new Error("jsPDF timeout")); }, 8000);
      return;
    }
...
async function genererRecuPDF(resa: Reservation, chauffeur: Chauffeur | null): Promise<void> {
  ...
  doc.setTextColor(...gray);
  doc.text(
    resa.paiement === "cb" ? "Paiement par carte bancaire" : "Paiement en espèces",
    14, y + 24
  );
  ...
  if (chauffeur) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...gray);
    doc.text(`${chauffeur.note_moyenne.toFixed(1)} / 5  (${chauffeur.nb_avis} avis)`, W - 14, y + 20, { align: "right" });
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "24px 20px calc(40px + env(safe-area-inset-bottom,0px))", display: "flex", flexDirection: "column", gap: 20, animation: "fadeIn 0.4s ease" }}>
        ...
        <div style={{ background: "#111120", borderRadius: 20, border: "1px solid #2a2a4a", padding: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: "rgba(245,200,66,0.1)", border: "1px solid rgba(245,200,66,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>
              📄
            </div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 15, color: "#f5f5f5" }}>Reçu de course</div>
              <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>PDF généré instantanément sur votre appareil</div>
            </div>
          </div>
          ...
          <div style={{ background: "#111120", borderRadius: 20, border: "1px solid #2a2a4a", padding: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(245,200,66,0.12)", border: "2px solid rgba(245,200,66,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0, overflow: "hidden" }}>
                {chauffeur.photo_url
                  ? <img src={chauffeur.photo_url} alt={chauffeur.prenom} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  : "👤"}
              </div>
              <div>
