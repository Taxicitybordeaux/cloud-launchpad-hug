...
interface TaxiPos { lat: number; lng: number; heading: number; }
...
function CoursePage() {
    ...
    if (destCoord) {
      getRoute([taxiPos.lat, taxiPos.lng], destCoord).then((route) => {
        ...
        routeLayer.current = L.polyline(route.coords, {
          color: "#f5c842", weight: 4, opacity: 0.7,
          dashArray: "10, 6",
        }).addTo(map);
  ...
  const toggleSuivi = useCallback(() => {
    if (suiviActif) { setSuiviActif(false); return; }
    ...
    <div style={{ position: "fixed", inset: 0, background: "#0a0a14", fontFamily: "'DM Sans',sans-serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", top: 16, left: 16, right: 16, zIndex: 100 }}>
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 10, height: 10, borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 0 3px rgba(34,197,94,0.3)", animation: "taxiPulse 1.5s ease-in-out infinite" }} />
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#22c55e", textTransform: "uppercase", letterSpacing: "0.08em" }}>Course en cours</div>
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 1 }}>Chrono : <span style={{ color: "#f5c842", fontWeight: 700 }}>{formatDuree(tempsEcoule)}</span></div>
              </div>
            </div>
            {eta !== null && (
              <div style={{ textAlign: "right" }}>
                <div style={{ fontFamily: "'Clash Display',sans-serif", fontSize: 22, fontWeight: 700, color: "#f5c842" }}>{eta} min</div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(245,200,66,0.12)", border: "1px solid rgba(245,200,66,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>📍</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>Destination</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#f5f5f5", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginTop: 2 }}>{resa.destination}</div>
          </div>
          {distanceRestante && (
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <div style={{ fontFamily: "'Clash Display',sans-serif", fontSize: 18, fontWeight: 700, color: "#f5c842" }}>{distanceRestante} km</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <button
            ...
            style={{ padding: "14px 10px", background: "#1a1a2e", border: "2px solid #2a2a4a", borderRadius: 14, color: "#f5f5f5", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minHeight: 64 }}
          >
          ...
          <button
            onClick={toggleSuivi}
            style={{ padding: "14px 10px", background: suiviActif ? "rgba(34,197,94,0.12)" : "#1a1a2e", border: `2px solid ${suiviActif ? "rgba(34,197,94,0.4)" : "#2a2a4a"}`, borderRadius: 14, color: suiviActif ? "#22c55e" : "#f5f5f5", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minHeight: 64 }}
          >
        ...
        {/* Info passager */}
        <div style={{ padding: "12px 14px", background: "#1a1a2e", borderRadius: 14, border: "1px solid #2a2a4a", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 11, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>Passager</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#f5f5f5", marginTop: 2 }}>{resa.prenom} {resa.nom}</div>
          </div>
          <a
            href={`tel:${resa.telephone}`}
            style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, textDecoration: "none" }}
          >
            📞
          </a>
        </div>
