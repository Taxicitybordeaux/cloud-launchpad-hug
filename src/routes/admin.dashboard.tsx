imimport { createFileRoute } from "@tanstack/react-router";
...
function Dashboard() {
  ...
  const fetchClients = useCallback(async () => {
    const { data: clientRows, error } = await supabase.from("clients").select("*").order("created_at", { ascending: false });
    if (error || !clientRows) { setClientsLoading(false); return; }
  ...
  const repairMissingPrices = useCallback(async (rows: any[]) => {
    ...
    for (const r of toRepair) {
      ...
      try {
        ...
        const prix = r.pickup_datetime
          ? calculerPrixMixte(km, r.pickup_datetime)
          : calculerPrix(km, true);
  ...
  const handleSendEmail = async (r: any) => {
    ...
    const prixCalcule = km
      ? (r.pickup_datetime ? calculerPrixMixte(km, r.pickup_datetime) : calculerPrix(km, true))
      : null;
            ...
            {pending.map((r) => {
              ...
              const prix =
                r.prix_estime != null
                  ...
                  : km_card != null
                    ? (r.pickup_datetime ? calculerPrixMixte(km_card, r.pickup_datetime) : calculerPrix(km_card, !tarif_nuit_card))
                    : null;
                          ...
                          <button
                            onClick={() => {
                              ...
                              const prixNum = r.prix_estime
                                ? Number(r.prix_estime)
                                : km_wa
                                  ? (r.pickup_datetime ? calculerPrixMixte(km_wa, r.pickup_datetime) : calculerPrix(km_wa, !tarif_nuit_wa))
                                  : null;
                    ...
                    {phone &&
                      (() => {
                        ...
                        const prixNum = r.prix_estime
                          ? Number(r.prix_estime)
                          : km_sms
                            ? (r.pickup_datetime ? calculerPrixMixte(km_sms, r.pickup_datetime) : calculerPrix(km_sms, !tarif_nuit_sms))
                            : null;
                        const prixStr = prixNum ? `${prixNum.toFixed(2)} €` : "à confirmer";
                        const refId = `TCB-${r.id.slice(0, 8).toUpperCase()}`;
                        return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 120 }}>
                      ...
                      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 4 }}>
                        {phone && (
                          <a href={`tel:${phone}`} style={{ color: "#0ea5e9", textDecoration: "none", fontSize: 13, fontWeight: 600 }}>
                            📞 {phone}
                          </a>
                        )}
                        {email && (
                          <a href={`mailto:${email}`} style={{ color: "#64748b", textDecoration: "none", fontSize: 13 }}>
                            ✉️ {email}
                          </a>
                        )}
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                      ...
                      >
                        ...
                        <div style={{ fontSize: 10, color: "#64748b", fontFamily: "'JetBrains Mono',monospace", letterSpacing: "0.06em" }}>
                          COURSES
                        </div>
                      </div>
                      <div
                      ...
                      >
                        <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 18, color: "#0ea5e9" }}>
              ...
              >
                {kmLoading ? (
                  ...
                  (() => {
                    ...
                    const px = rv.pickup_datetime
                      ? calculerPrixMixte(km, rv.pickup_datetime)
                      : calculerPrix(km, true);
                    return (
                      <>
                        <div>
